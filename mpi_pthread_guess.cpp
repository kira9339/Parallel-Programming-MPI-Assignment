...
void PriorityQueue::init()
{
    ...
    // 第一步：估算每个 PT 的猜测量（= 最后一个 segment 的 value 数）
    vector<pair<int, long long>> pt_workload;  // <pt_idx, estimated_guesses>
    for (int i = 0; i < (int)m.ordered_pts.size(); i++) {
        const PT& pt = m.ordered_pts[i];
        long long est = 0;
        if (pt.content.back().type == 1)
            est = m.letters[m.FindLetter(pt.content.back())].ordered_values.size();
        else if (pt.content.back().type == 2)
            est = m.digits[m.FindDigit(pt.content.back())].ordered_values.size();
        else
            est = m.symbols[m.FindSymbol(pt.content.back())].ordered_values.size();
        pt_workload.push_back({ i, est });
    }

    // 第二步：按猜测量降序排列（大的先分配，避免大任务集中到最后）
    sort(pt_workload.begin(), pt_workload.end(),
        [](auto& a, auto& b) { return a.second > b.second; });

    // 第三步：贪心分配——每个 PT 分配给当前负载最小的进程
    vector<long long> process_load(size, 0);

    for (auto& [pt_idx, workload] : pt_workload)
    {
        // 所有进程独立计算，结果完全一致！
        int target = min_element(process_load.begin(), process_load.end()) - process_load.begin();
        process_load[target] += workload;

        if (target == rank)
        {
            // 这个 PT 归我！初始化它
            PT pt = m.ordered_pts[pt_idx];
            for (segment seg : pt.content) {
                if (seg.type == 1)
                    pt.max_indices.emplace_back(m.letters[m.FindLetter(seg)].ordered_values.size());
                else if (seg.type == 2)
    ...
    if (rank == 0) {
        cout << "Process loads: ";
        for (int i = 0; i < size; i++) cout << process_load[i] << " ";
        cout << endl;
        cout << "priority size: " << priority.size() << endl;
    }
}
...
inline void* MultiThread(void* arg) {
 ...
 void priorityqueue::generate(pt pt)
 {
     ...
     if (pt.content.size() == 1)
     {
         ...
         // 启动 thread_id 个线程并行填充
         pthread_t threads[thread_id];
         threadarg args[thread_id];
         int chunk = total / thread_id;

         for (int t = 0; t < thread_id; t++) {
             args[t].start = t * chunk;
             args[t].end = (t == thread_id - 1) ? total : (t + 1) * chunk;
             args[t].base = old_size;
             args[t].st = a;
             args[t].q = this;
             pthread_create(&threads[t], null, singlethread, &args[t]);
         }
     ...
     else
     {
         ...
         for (int t = 0; t < thread_id; t++) {
             ...
             args[t].last_st = a;
             args[t].q = this;
             pthread_create(&threads[t], null, multithread, &args[t]);
         }
